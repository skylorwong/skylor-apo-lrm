...
class ProTeGi(PromptOptimizer):
    ...
    def _get_gradients(self, prompt, error_string, num_feedbacks=5, n=1):
        ...
        gradient_engine = self.opt['gradient_engine']
        if 'gpt' in gradient_engine.lower():
            res = utils.chatgpt(gradient_prompt, n=n, model=gradient_engine)
        elif 'gemini' in gradient_engine.lower():
            res = utils.gemini(gradient_prompt, n=n, model_name=gradient_engine)
        feedbacks = []
        new_prompts = []
        for r in res:    
...
class ProTeGiV2(PromptOptimizer):
    """ ProTeGi: Prompt Optimization with Textual Gradients
    """
    def _sample_error_str(self, texts, labels, preds, labels_parsed, preds_parsed, task, n=4):
        """ Sample n error strings from the given texts, labels, and preds"""
        error_idxs = []
        for i, (l, p) in enumerate(zip(labels_parsed, preds_parsed)):
        ...
        sample_texts = [texts[i] for i in sample_idxs]
        sample_labels = [labels_parsed[i] for i in sample_idxs]
        sample_preds = [preds[i][0] for i in sample_idxs]
        error_string = ''
        error_idx = 0
        for i, (t, l, p) in enumerate(zip(sample_texts, sample_labels, sample_preds)):
            error_string += f'## Example {error_idx+1}\n'
            error_string += f'Question: \"{t.strip()}\"\nFinal Answer: {task.stringify_prediction(l)}\nResponse: {p}\n\n'
            error_idx += 1
        return error_string.strip()
    ...
    def _get_gradients(self, prompt, error_string, num_feedbacks=5, n=1):
        """ Get "gradients" for a prompt based on the error string."""
        if self.opt['task'] == 'math':
            gradient_prompt = prompts.math
        else:
        ...
        if 'gpt' in gradient_engine.lower():
            # res = utils.chatgpt(gradient_prompt, n=n, model=gradient_engine)
            res = utils.gpt(gradient_prompt, n=n, model=gradient_engine, max_tokens=4096)
            print("ERROR: RUNNING GPT NOT GEMINI")
        elif 'gemini' in gradient_engine.lower():
            res = utils.gemini(gradient_prompt, n=n, max_tokens=4096, model_name=gradient_engine)
        feedbacks = []
        new_prompts = []
        for r in res:    
            parse_tagged_text = self.parse_tagged_text(r, "<START>", "<END>")
            if not parse_tagged_text:
                parse_tagged_text = self.parse_tagged_text(r, "<START>", "</END>")
            elif not parse_tagged_text:
                parse_tagged_text = self.parse_tagged_text(r, "<START/>", "<END>")
            elif not parse_tagged_text:
                parse_tagged_text = self.parse_tagged_text(r, "<START/>", "</END>")
            feedbacks += parse_tagged_text
        
        with open(self.opt['out'], 'a') as outf:
            outf.write('feedbacks: ' + json.dumps(feedbacks) + '\n')
    ...
    def apply_gradient(self, prompt, error_str, feedback_str, steps_per_gradient, n=1):
        """ Incorporate feedback gradient into a prompt."""
        if self.opt['task'] == 'math':
            transformation_prompt = prompts.math
        transformation_prompt += f"""
        My current prompt is:
        "{prompt}"
        ...
        transformation_prompt = '\n'.join([line.lstrip() for line in transformation_prompt.split('\n')])
        # res = utils.chatgpt(transformation_prompt, n=n)
        # res = utils.gpt(transformation_prompt, n=n, model=self.opt['gradient_engine'], max_tokens=4096)
        res = utils.gemini(transformation_prompt, n=n, model_name=self.opt['gradient_engine'], max_tokens=4096)
        new_prompts = []
        for r in res:   
            parse_tagged_text = self.parse_tagged_text(r, "<START>", "<END>")
            if not parse_tagged_text:
    ...
    def generate_synonyms(self, prompt_section, n=3):
        ...
        rewriter_prompt = f"Generate a variation of the following instruction while keeping the semantic meaning.\n\nInput: {prompt_section}\n\nOutput:"
        # new_instructions = utils.chatgpt(rewriter_prompt, n=n)
        #new_instructions = utils.gpt(rewriter_prompt, n=n, model=self.opt['gradient_engine'], max_tokens=4096)
        new_instructions = utils.gemini(rewriter_prompt, n=n, model_name=self.opt['gradient_engine'], max_tokens=4096)
        new_instructions = [x for x in new_instructions if x]
        return new_instructions

    def get_gradients(self, prompt, task_section, task, gpt4, texts, labels, preds, labels_parsed=None, preds_parsed=None):
        """ Get "gradients" for a prompt based on sampled error strings."""
        prompt_feedbacks = []
        for _ in tqdm(range(self.opt['n_gradients']), total=self.opt['n_gradients'], desc='gradients..'):
            error_string = self._sample_error_str(
                texts, labels, preds, labels_parsed, preds_parsed, task, n=self.opt['errors_per_gradient'])
            gradients = self._get_gradients(
                task_section, error_string, self.opt['gradients_per_error'], n=1)
            prompt_feedbacks += [(t, error_string) for t in gradients]
        return prompt_feedbacks
    ...
    def expand_candidates(self, prompts, task, gpt4, train_exs):
        ...
        for prompt in tqdm(prompts, desc=f'expanding {len(prompts)} prompts'):
            ...
            # evaluate prompt on minibatch
            _, texts, labels, preds, labels_parsed, preds_parsed = task.evaluate(gpt4, prompt, minibatch)

            # get gradients
            new_task_sections = []
            if self.opt['n_gradients'] > 0:
                gradients = self.get_gradients(prompt, task_section, task, gpt4, texts, labels, preds, labels_parsed, preds_parsed)
                new_task_sections = []
                for feedback, error_string in tqdm(gradients, desc='applying gradients'):
                    tmp = self.apply_gradient(
                        task_section, error_string, feedback, self.opt['steps_per_gradient'])
